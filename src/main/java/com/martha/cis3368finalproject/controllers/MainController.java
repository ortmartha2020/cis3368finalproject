...
public class MainController {
    ...
    public ModelAndView save(@RequestParam("id") String id, @RequestParam("country_name")String country_name,
        ...
        chartData covidData;
        if (!id.isEmpty()){
            Optional<chartData> data = ChartDataRepo.findById(id);
            covidData = data.get();
        }
        else
        {
            covidData = new chartData();
            covidData.setId(UUID.randomUUID().toString());
        }
        covidData.setCountry_name(country_name);
        covidData.setDate(date);
        covidData.setTotal_cases(total_cases.toString());
        covidData.setTotal_deaths(total_deaths.toString());
        covidData.setNew_cases(new_cases.toString());
        covidData.setUser_id(user_id);
        return mv;
    }
